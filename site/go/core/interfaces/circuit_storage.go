...
// CircuitStorageInterface Represents an interface to a circuit storage system
// that simply performs the requested operation.
// Implementations do not modify the provided circuits.
...
// CircuitStorageInterfaceFactory Generates interfaces to a storage systems,
// but each interface returned should be associated with the same storage
// system.  This is not required to generate unique instances if that is not
// required by the underlying storage system
